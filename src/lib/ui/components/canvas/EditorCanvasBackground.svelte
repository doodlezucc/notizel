<script lang="ts">
	import type { CameraTransform } from '$lib/data/common';

	interface Props {
		transform: CameraTransform;
	}

	let { transform }: Props = $props();

	let contentRect = $state<DOMRect>();
</script>

<div class="background" bind:contentRect>
	{#if contentRect}
		<svg width={contentRect.width} height={contentRect.height}>
			<defs>
				<pattern
					id="pattern"
					width="40"
					height="40"
					patternUnits="userSpaceOnUse"
					patternTransform="translate({contentRect.width / 2} {contentRect.height /
						2}) scale({transform.scale}) translate({transform.position.x} {transform.position.y})"
				>
					<!-- "Hideout" from https://heropatterns.com/ -->
					<path
						d="M0 38.59l2.83-2.83 1.41 1.41L1.41 40H0v-1.41zM0 1.4l2.83 2.83 1.41-1.41L1.41 0H0v1.41zM38.59 40l-2.83-2.83 1.41-1.41L40 38.59V40h-1.41zM40 1.41l-2.83 2.83-1.41-1.41L38.59 0H40v1.41zM20 18.6l2.83-2.83 1.41 1.41L21.41 20l2.83 2.83-1.41 1.41L20 21.41l-2.83 2.83-1.41-1.41L18.59 20l-2.83-2.83 1.41-1.41L20 18.59z"
					/>
				</pattern>
			</defs>

			<rect fill="url(#pattern)" width={contentRect.width} height={contentRect.height}></rect>
		</svg>
	{/if}
</div>

<style lang="scss">
	@use '$lib/ui/style/scheme.scss';

	.background {
		position: relative;
		background-color: transparent;
	}

	svg {
		position: absolute;
	}

	path {
		fill: scheme.$background-pattern;
	}
</style>
